...
const PROVIDERS = [
  ...
  provider('baseten', 'Baseten', 'https://inference.baseten.co/v1', 'moonshotai/Kimi-K3', 1048576, { apiKeyUrl: 'https://docs.baseten.co/inference/model-apis/overview', supportsVision: true, inputCostPerMillionUsd: 3, cacheReadCostPerMillionUsd: 0.3, outputCostPerMillionUsd: 15 }),
  ...
  provider('cohere', 'Cohere', 'https://api.cohere.ai/compatibility/v1', 'command-a-plus-05-2026', 128000, { category: 'cloud', apiKeyUrl: 'https://dashboard.cohere.com/api-keys', inputCostPerMillionUsd: 2.5, outputCostPerMillionUsd: 10, suggestions: ['command-a-plus-05-2026', 'command-a-03-2025'] }),
  ...
  provider('deepinfra', 'DeepInfra', 'https://api.deepinfra.com/v1/openai', 'Qwen/Qwen3.8-27B', 262144, { apiKeyUrl: 'https://deepinfra.com/dash/api_keys', supportsVision: true, inputCostPerMillionUsd: 0.1, outputCostPerMillionUsd: 0.3 }),
  ...
  provider('google-vertex', 'Google Vertex AI', 'https://{vertex_endpoint}/v1/projects/{project}/locations/{location}/endpoints/openapi', 'gemini-3.7-flash', 1048576, { category: 'cloud', apiKeyUrl: 'https://console.cloud.google.com/apis/credentials', apiKeyHeader: 'x-goog-api-key', project: '', location: 'us-central1', supportsVision: true, apiKeyPlaceholder: 'Google authorization key', suggestions: ['gemini-3.1-pro', 'gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.5-flash'] }),
  provider('google-vertex-anthropic', 'Google Vertex AI (Anthropic)', 'https://{vertex_endpoint}', 'claude-sonnet-5', 1000000, { category: 'cloud', type: 'vertex_anthropic', apiKeyUrl: 'https://console.cloud.google.com/apis/credentials', project: '', location: 'us-east5', supportsVision: true, inputCostPerMillionUsd: 2, cacheReadCostPerMillionUsd: 0.2, cacheWriteCostPerMillionUsd: 2.5, outputCostPerMillionUsd: 10, apiKeyPlaceholder: 'Google authorization key', suggestions: ['claude-opus-5', 'claude-sonnet-5', 'claude-haiku-4-5@20251001'] }),
  provider('helicone', 'Helicone', 'https://ai-gateway.helicone.ai/v1', 'chatgpt-4o-latest', 128000, { apiKeyUrl: 'https://helicone.ai/', supportsVision: true, inputCostPerMillionUsd: 5, cacheReadCostPerMillionUsd: 2.5, outputCostPerMillionUsd: 20 }),
  ...
  provider('minimax-cn-coding-plan', 'MiniMax Token Plan (China)', 'https://api.minimaxi.com/v1', 'MiniMax-M3', 1000000, { category: 'cloud', apiKeyUrl: 'https://platform.minimaxi.com/docs/token-plan/intro', supportsVision: true, suggestions: ['MiniMax-M3', 'MiniMax-M2.7'] }),
  provider('minimax-coding-plan', 'MiniMax Token Plan', 'https://api.minimax.io/v1', 'MiniMax-M3', 1000000, { category: 'cloud', apiKeyUrl: 'https://platform.minimax.io/docs/token-plan/intro', supportsVision: true, suggestions: ['MiniMax-M3', 'MiniMax-M2.7'] }),
  ...
  provider('modelscope', 'ModelScope', 'https://api-inference.modelscope.cn/v1', 'Qwen/Qwen3.8-27B', 262144, { apiKeyUrl: 'https://modelscope.cn/docs/model-service/API-Inference/intro', supportsVision: true }),
  ...
  provider('nearai', 'NEAR AI Cloud', 'https://cloud-api.near.ai/v1', 'z-ai/glm-5.3-flash', 1048576, { apiKeyUrl: 'https://cloud.near.ai/', supportsVision: true, inputCostPerMillionUsd: 0.15, cacheReadCostPerMillionUsd: 0.035, outputCostPerMillionUsd: 0.5, suggestions: ['z-ai/glm-5.3-flash', 'Qwen/Qwen3.8-27B'] }),
  ...
  provider('opencode', 'OpenCode Zen', 'https://opencode.ai/zen/v1', 'muse-spark-1.2-contributor-free', 262000, { apiKeyUrl: 'https://opencode.ai/docs/zen', suggestions: ['muse-spark-1.2-contributor-free', 'mimo-v2.5-free', 'ling-3.0-flash-fin-free', 'nemotron-3-ultra-free', 'nemotron-3.5-lightning-free', 'big-pickle'], inputCostPerMillionUsd: 0, outputCostPerMillionUsd: 0, cacheReadCostPerMillionUsd: 0 }),
  ...
  provider('pollinations', 'Pollinations AI', 'https://gen.pollinations.ai/v1', 'openai', 131072, { category: 'router', apiKeyUrl: 'https://enter.pollinations.ai/keys', supportsVision: true, supportsAskStreaming: true, supportsStreamUsageOptions: true, suggestions: ['openai', 'claude', 'gemini', 'qwen3.8-max'] }),
  ...
  provider('siliconflow', 'SiliconFlow', 'https://api.siliconflow.com/v1', 'moonshotai/Kimi-K3', 1048576, { apiKeyUrl: 'https://cloud.siliconflow.com/account/ak', supportsVision: true, inputCostPerMillionUsd: 3, cacheReadCostPerMillionUsd: 0.3, outputCostPerMillionUsd: 15 }),
  ...
  provider('stepfun', 'StepFun', 'https://api.stepfun.com/v1', 'step-3.7-flash', 262144, { category: 'cloud', apiKeyUrl: 'https://platform.stepfun.com/docs/zh/overview/concept', supportsVision: true, inputCostPerMillionUsd: 2.05, cacheReadCostPerMillionUsd: 0.41, outputCostPerMillionUsd: 9.59, suggestions: ['step-3.7-flash', 'step-3.5-flash'] }),
  ...
  provider('vercel', 'Vercel AI Gateway', 'https://ai-gateway.vercel.sh/v1', 'xai/grok-4.1-fast-reasoning', 1000000, { apiKeyUrl: 'https://vercel.com/ai-gateway', supportsVision: true, inputCostPerMillionUsd: 0.2, cacheReadCostPerMillionUsd: 0.05, outputCostPerMillionUsd: 0.5 }),
  ...
  provider('zai-coding-plan', 'Z.AI Coding Plan', 'https://api.z.ai/api/coding/paas/v4', 'glm-5.3', 1000000, { category: 'cloud', apiKeyUrl: 'https://docs.z.ai/devpack/overview', suggestions: ['glm-5.3', 'glm-5.2'] }),
  ...
  provider('zhipuai', 'Zhipu AI', 'https://open.bigmodel.cn/api/paas/v4', 'glm-5.3', 1000000, { category: 'cloud', apiKeyUrl: 'https://open.bigmodel.cn/usercenter/apikeys', inputCostPerMillionUsd: 1.4, cacheReadCostPerMillionUsd: 0.26, outputCostPerMillionUsd: 4.4, suggestions: ['glm-5.3', 'glm-5.2', 'glm-5.1'] }),
  provider('zhipuai-coding-plan', 'Zhipu AI Coding Plan', 'https://open.bigmodel.cn/api/coding/paas/v4', 'glm-5.3', 1000000, { category: 'cloud', apiKeyUrl: 'https://docs.bigmodel.cn/cn/coding-plan/overview', suggestions: ['glm-5.3', 'glm-5.2'] }),
];
