...
export default {
  ...
  "st.providers.webbrain_data_use.body": "WebBrain Cloud には毎日の無料利用枠が含まれます。「WebBrain の改善に協力する」は既定でオンであり、選択された Cloud の会話が保持され、評価、改善、微調整、トレーニングに使用される場合があります。今後の Cloud 操作をこれらの用途から除外するには、一般 → 詳細設定でオフにしてください。<u>ローカルモデルへのリクエストや、ご自身の API 認証情報で直接送信したリクエストを WebBrain が収集することはありません。</u>{privacyLink}。追加利用は {subscribeLink} で購読してください。請求は {accountLink} で管理できます。",
