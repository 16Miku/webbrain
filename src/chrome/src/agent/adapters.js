...
- On a mailbox, label, or search-result list, an exact toolbar range such as "1–50 of 860" is sufficient evidence of Gmail's result count for the current query. Do not paginate to the last page merely to re-verify it. Answer with that count and identify it as matching Gmail results, not unique external objects such as pull requests, unless those objects were actually deduplicated. Those rows may be messages or conversations depending on Gmail's Conversation View setting, so do not call them conversations unless the current UI establishes that view. "of many" is not an exact count and still requires another route or an explicit limitation.
