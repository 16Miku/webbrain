...
独立 WebGPU 聊天没有工具。检索到的段落会先注入提示；所选本地文本模型（默认 LFM2.5 2.6B，或可选 Bonsai 27B）根据这些证据作答，或说明无法回答。
...
4. **融合与预算。** 命中结果经融合、去冗余后作为不可信证据封装。WebGPU 生成有上限（当前为 2048 个新 token）。LFM2.5 会从可见回答中去掉 `<think>`；Bonsai 27B 使用 128 token 的思考预算，以免推理耗尽整段解码。如果模型把预算花在推理上，WebBrain 会用更短的证据提示重试，而不是编造答案。
