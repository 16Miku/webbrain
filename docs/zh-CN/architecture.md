...
同一标签页的普通对话会按 `conversationId` 保留消息历史；每轮请求会把这份历史（必要时经过压缩）重新发送给模型。侧边栏显示的 `tabChat:<tabId>` 是可见 HTML，`agentConv:<tabId>` 才是提供商收到的模型对话，两者不能简单视为同一份内容。

选中文本会建立持久的 `source_grounding` 边界：固定动作（总结、翻译、校对等）使用 `selection_only`，只发送当前选区；自由提问可使用 `selection_context`，在当前选区和模型常识之外，只投影早期的用户问题与助手回答，用于解析“上述”“这三者”等指代。早期选区原文、工具结果、截图、附件和应用内部状态不会跨越边界；助手回答只是先前答案，不是已验证来源，也不是可执行指令。

因此，界面中仍可见的气泡不一定全部进入模型请求。每个选区会在消息流中显示作用域分隔线，顶部提示会说明包含和排除的内容。用户确认“恢复完整对话”后，作用域才会解除，下一轮才重新发送完整对话；“新对话”则清除消息和作用域。作用域状态会随会话存储跨越面板重载、Service Worker 重启、重试、标签页切换和上下文压缩，并在清除对话时一并删除。

标准 Trace 的运行时元数据只记录作用域策略、锚点是否存在和被排除消息数量，不记录投影文本本身；因此默认 Trace 可以解释模型为什么看见或看不见某轮消息，又不会扩大敏感内容暴露面。内存中的深度调试环和用户主动开启的无损 Trace 是独立的诊断面，仍保留原有请求/响应内容；导出它们属于明确且需要注意隐私的诊断操作。
