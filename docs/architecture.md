...
| `read_email_verification_message` | service worker + bounded accessibility reads | Mid/Full only after the OTP skill is active; directly scopes verified already-open message routes and uses a disposable inactive clone only to open one inspected inbox item |
