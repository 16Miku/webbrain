...
| `read_email_verification_message` | service worker + bounded accessibility reads | Mid/Full only after the OTP skill is active; internally selects an already-open supported mailbox and uses a disposable inactive clone only to open one inspected message |
