...
| **TypeSafe System One sidecar** | Optional scheduled-task judging sends only bounded task/observation state to the documented TypeSafe endpoint. It is off by default, requires an explicit global switch, a per-feature switch, and a user-provided API key. It has no browser tools and can only downgrade an existing success to partial. Strict Secret Mode, cancellation, cost-limit runs, malformed responses, and transport failures skip or fail open. |
