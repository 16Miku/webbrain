...
| `notifications` | Can show OS-level notifications | Used for run-completion, download, and scheduled-task status. Notifications carry short status text only — never credentials, page bodies, or trace payloads. |
...
| `http://localhost/*`, `http://127.0.0.1/*`, `http://*/*` host permissions | Lets the extension call local-model servers and plain-HTTP endpoints | Localhost entries exist for local LLM servers (llama.cpp, Ollama, LM Studio). Plain-HTTP is for LAN devices and local servers that lack TLS — all such fetches still pass the capability × origin permission prompt, and `connect-src *` in the extension CSP does not waive that gate. |
