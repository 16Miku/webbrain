...
### Fixed
- Prevented direct-message sends on protected messaging routes unless the planner carries the user-authorized recipient and a read-only pre-dispatch probe verifies one unique, exact active-conversation header identity. Active-conversation requests are pinned to that identity before any page tool runs; ordinary conversation text never counts as recipient evidence, search-field Enter stays distinct from composer submission, and unresolved or distant controls plus dispatch paths that cannot bind to the verified recipient—including attachment injection that may auto-send—fail closed. The first enforced adapter is Douyin chat, with Chrome/Firefox parity.
